 ...
 * 
 * @help
*이 플러그인은 한글을 입력 할 수있는 키보드를 제공합니다.
 * 또는 이름 입력 처리의 다른 모국어.
 *
 * =============================================== ==========================
 * 플러그인 명령
 * =============================================== ==========================
 *
 *이 플러그인 명령은 이름 창의 속성을 변경할 수 있습니다.
 *
 * KNE width number
 ...
 * KNE askText string
 *
 * 이름을 9 자 이상 입력하려면 다음 플러그인 명령을 호출해야합니다.
 *
 * OpenXNameInput 리더 9
 *
 * =============================================== ==========================
 * 변경 로그
 * =============================================== ==========================
 * 2016.03.05 (v1.3.3)-클래스 구조 수정.
 * 2016.03.22 (v1.4.0)-심각한 문제를 일으키는 버그 수정.
 * 2016.04.05 (v1.5.0)-텍스트가 자동으로 삭제되는 버그 수정
 * 길이가 2 미만인 한글 텍스트를 입력 할 수있는 경우
 * 2016.06.18 (v1.6.0)-상속 구조 및 'askText'라는 매개 변수 수정.
 * 2016.08.09 (v1.6.1)-Input 클래스의 shouldPreventDefault 함수 수정.
 * 2016.12.20 (v1.6.2) : Default CharWidth 매개 변수가 추가되었습니다.
 * 이는이 플러그인에 'navigator.language'가 항상 반환하는 버그가 있기 때문입니다.
 * crosswalk-10.39.235.16 xwalk 라이브러리의 버그로 인한 'en-US'. 그래서 이것을 추가했습니다
 * 잘못된 문자 너비를 반환하는 문제를 해결합니다.
 * 2017.01.13 (v1.6.3)-android에서 onchange 이벤트 발생 후 상태 표시 줄을 숨기지 않는 버그 수정.
 * 2017.06.07 (v1.6.4) :
 *-기본 배경 이미지를 변경할 수있는 새로운 기능이 추가되었습니다.
 *-안드로이드에서 취소 버튼을 누른 후 텍스트가 변경되지 않는 문제 수정.
 *-이제 편집 버튼을 누르지 않으면 텍스트 편집기의 포커스를 얻지 못합니다.
 * 2017.09.15 (v1.6.5) :
 *-이름 처리시 기본 이름을 미리 설정하는 새로운 기능이 추가되었습니다.
 * 2018.10.26 (v1.6.6) :
 *-경고창을 추가했습니다.
 * 2018.11.08 (v1.6.7) :
 *-배우의 이름은 이전 이름과 동일하게 변경 될 수 있습니다.
 *-이제 명령 버튼이 모바일 장치에서만 표시됩니다.
 *-편집 창의 위치를 ​​수정했습니다.
 * 2019.05.30 (v1.6.8) :
 *-이름을 9 자 이상 입력 할 수있는 기능이 추가되었습니다.
 * 2019.11.23 (v1.6.9) :
 *-입력 방법 편집기가 화면에 표시되는지 여부를 선택할 수있는 새로운 기능이 추가되었습니다.
 * 그 기능은 모바일 장치에서 많은 문제를 해결할 수 있습니다.
 *-이름 편집 창에서 얼굴 이미지를 숨길 수있는 새로운 기능이 추가되었습니다.
 * 2020.07.14 (v1.6.10) :
 *-문제 수정, https://github.com/biud436/MV/issues/18
 * 2021.01.11 (v1.6.11) :
 *-이제 요청 텍스트에 텍스트 코드를 사용할 수 있습니다.
 */
/*~struct~TextBox:
 * 
 * @param width
